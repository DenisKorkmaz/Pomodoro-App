...
function WorkLogPage() {
  ...
  return (
    <div>
      ...
      <h1 className="date-info">
        Your Work Log for CW {currentWeekNumber} ({weekRange.start} -{" "}
        {weekRange.end})
      </h1>

      <ul>
        {Object.keys(
          (workLog[currentYear] && workLog[currentYear][currentWeekNumber]) ||
            {}
        ).map((category) => {
          if (selectedCategory === "all" || selectedCategory === category) {
            ...
            return (
              <li key={category}>
                You worked {formatTime(timeWorked)} on {category} this week.
              </li>
            );
          }
          return null;
